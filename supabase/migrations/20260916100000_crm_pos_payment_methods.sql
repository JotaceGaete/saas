-- ============================================================
-- TPV-PAYMENT-METHODS-1 — crm_create_pos_sale rechaza los medios de pago
-- desglosados introducidos por CAJA-CIERRE-CONCILIACION-1 (PR #69).
--
-- Bug real en producción: una venta TPV pagada con Débito devuelve HTTP
-- 400 { "code": "23514", "message": "INVALID_PAYMENT" }. El frontend
-- (CrmTerminal.jsx, PAYMENT_METHODS) ya ofrece debit_card/credit_card/
-- mercado_pago desde PR #69, y crmService.js los pasa sin tocar
-- (normalizePaymentMethod los auto-mapea a sí mismos -- ver ese archivo),
-- pero la validación de vocabulario DENTRO de crm_create_pos_sale nunca
-- se actualizó cuando se agregaron esos medios: sigue siendo
--   v_method NOT IN ('cash', 'card', 'bank_transfer', 'check', 'other')
-- (definición vigente desde 20260911000000_tpv_stock_insufficient_details.sql,
-- la última migración que tocó esta función -- confirmado por grep: ningún
-- otro archivo la redefine desde entonces). 'debit_card' no está en esa
-- lista => rechazado con INVALID_PAYMENT aunque sea un medio real y válido
-- para el TPV actual.
--
-- Causa raíz exacta: una sola línea, el IF de vocabulario de medios de
-- pago dentro del loop de validación de p_payments. Nada más en la
-- función está roto.
--
-- Esta migración NO se creó editando 20260910220000/20260911000000 (ya
-- aplicadas) -- es un CREATE OR REPLACE FUNCTION nuevo, con la MISMA
-- firma exacta, que reemplaza el cuerpo in place preservando TODO lo
-- demás byte a byte respecto de la versión vigente (20260911000000):
--   * mismos locks (pg_advisory_xact_lock, FOR UPDATE sobre wa_products
--     y sobre crm_cash_sessions);
--   * misma idempotencia por (business_id, pos_idempotency_key);
--   * mismas validaciones (NOT_AUTHENTICATED, MISSING_REQUIRED_PARAMETER,
--     ownership, CUSTOMER_NOT_FOUND, INVALID_ITEMS, PRODUCT_NOT_FOUND);
--   * mismo mensaje STOCK_INSUFFICIENT:<product_id>:<requested>:<available>
--     de TPV-STOCK-UX-1, sin modificar;
--   * misma regla "solo efectivo genera vuelto" (INVALID_PAYMENT si un
--     medio no-cash supera el total) -- IF v_method <> 'cash' ya agrupaba
--     genéricamente TODO lo no-efectivo, así que los medios nuevos entran
--     solos a esa lógica sin ningún cambio adicional;
--   * mismo cálculo/aplicación de efectivo, mismos inserts de
--     crm_invoices/crm_invoice_items/crm_payments/crm_stock_movements,
--     mismo orden, mismos GRANT/REVOKE, mismo SECURITY DEFINER,
--     search_path y comentario de función (solo se amplía el listado de
--     medios documentado).
--
-- Único cambio real: el vocabulario aceptado pasa de
--   ('cash', 'card', 'bank_transfer', 'check', 'other')
-- a
--   ('cash', 'card', 'debit_card', 'credit_card', 'bank_transfer',
--    'mercado_pago', 'check', 'other')
-- -- exactamente el mismo vocabulario que ya reconoce la conciliación de
-- caja (crm_close_cash_session, 20260915180000_crm_cash_session_reconciliations.sql).
-- 'card' se conserva sin tocar (compatibilidad con ventas históricas: una
-- venta nueva pagada con tarjeta ahora usa debit_card/credit_card, pero
-- 'card' sigue siendo un valor válido de payment_method, nunca se
-- reclasifica nada existente). 'credit' (cuenta corriente) DELIBERADAMENTE
-- NO se agrega acá -- sigue sin ser un medio de pago válido para esta RPC,
-- exactamente igual que antes: cuenta corriente en un TPV se expresa como
-- saldo pendiente (paid_total < total) + p_customer_id obligatorio, nunca
-- como una línea de p_payments con method='credit'. Confundir
-- 'credit_card' con 'credit' rompería esa semántica -- no se hace.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    --
    -- TPV-STOCK-UX-1: el mensaje lleva product_id, cantidad solicitada y
    -- stock disponible, para que el frontend pueda señalar exactamente
    -- qué producto causó el rechazo. product_id nunca es NULL en este
    -- punto (el WHERE del cursor ya lo garantiza), así que siempre es un
    -- UUID válido para el formato "STOCK_INSUFFICIENT:<uuid>:<int>:<int>".
  ...
  LOOP
    ...
    -- TPV-PAYMENT-METHODS-1: vocabulario ampliado para que coincida con
    -- CrmTerminal.jsx (PAYMENT_METHODS) y con la conciliación de caja
    -- (crm_close_cash_session) -- 'card' se conserva por compatibilidad
    -- histórica, 'credit' (cuenta corriente) sigue sin ser un medio de
    -- pago válido acá a propósito.
  ...
  -- paid_total de arriba, ahora insertando cada fila real. payment_method
  -- se persiste EXACTAMENTE como llegó en v_method (ya normalizado a
  -- minúsculas/trim) -- ninguna reclasificación adicional acá, así que
  -- debit_card/credit_card/mercado_pago quedan guardados tal cual.
  v_remaining_for_cash := GREATEST(v_total - v_non_cash_total, 0);
  FOR v_payment IN SELECT value FROM jsonb_array_elements(COALESCE(p_payments, '[]'::jsonb))
  LOOP
    v_method := lower(trim(COALESCE(v_payment->>'method', '')));
...
COMMENT ON FUNCTION public.crm_create_pos_sale(UUID, TEXT, JSONB, DATE, UUID, NUMERIC, JSONB, TEXT, TEXT) IS
  ...
  'corriente no toca caja. Medios de pago aceptados (TPV-PAYMENT-METHODS-1): '
  'cash, card, debit_card, credit_card, bank_transfer, mercado_pago, check, '
  'other -- card se conserva por compatibilidad histórica, credit (cuenta '
  'corriente) nunca es un medio de pago válido acá. No maneja anulación/'
  'restock -- eso no existe todavía para ninguna venta de crm_invoices en '
  'este repo.';
