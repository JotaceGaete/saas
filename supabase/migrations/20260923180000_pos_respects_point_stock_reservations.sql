-- POINT-SMART-2-6B — hacer que la venta TPV normal respete reservas Point.
-- Reemplaza la última versión vigente (TPV-PAYMENT-METHODS-1) cambiando solo
-- la comprobación de disponibilidad: stock_actual - reservas Point activas.
-- Toda la semántica restante permanece idéntica.
...
BEGIN
  ...
  LOOP
    ...
    IF v_stock_actual IS NOT NULL THEN
      -- POINT-SMART-2-6: stock_actual todavía incluye las unidades reservadas
      -- por clientes que están pagando en Point. El helper resta esas
      -- reservas y conserva el mismo error detallado del TPV.
      PERFORM public.crm_point_assert_stock_available(
        p_business_id, v_agg.product_id, v_agg.total_qty::integer
      );
    END IF;
  END LOOP;
...
COMMENT ON FUNCTION public.crm_create_pos_sale(UUID, TEXT, JSONB, DATE, UUID, NUMERIC, JSONB, TEXT, TEXT) IS
  'POINT-SMART-2-6 + TPV-CORE-1: crea una venta de CrmTerminal (crm_invoices + crm_invoice_items '
