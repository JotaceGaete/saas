...
const STATUS_OPTIONS = [
  ...
  { key: 'hidden',    label: 'Ocultar',    icon: 'EyeOff' },
];

function PosSwitch({ active, onClick }) {
  return (
    <button
      type="button"
      role="switch"
      aria-checked={active}
      onClick={onClick}
      className="relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1"
      style={{ backgroundColor: active ? 'var(--color-primary)' : '#d1d5db' }}
      title={active ? 'Quitar del TPV' : 'Mostrar en TPV'}
    >
      <span
        className="pointer-events-none inline-block h-3.5 w-3.5 rounded-full bg-white shadow-sm transform transition-transform duration-200"
...
export default function ProductTable({
  ...
  return (
    <>
            ...
            <thead>
              <tr className="border-b" style={{ backgroundColor: 'rgba(248,250,252,0.8)', borderColor: 'rgba(226,232,240,0.7)' }}>
                ...
                <th className="px-4 py-4 text-left w-16">
                  <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>Imagen</span>
                </th>
                <th className="px-3 py-4 text-left w-[88px]">
                  <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>Código</span>
                </th>
                ...
                <th className="w-[120px] px-4 py-4 text-center">
                  <span className="text-xs font-semibold uppercase tracking-wide whitespace-nowrap" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>TPV</span>
                </th>
                <th className="w-[220px] px-4 py-4 text-right">
                  <span className="text-xs font-semibold uppercase tracking-wide whitespace-nowrap" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>Acciones</span>
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr
                    ...
                    className={`border-b last:border-0 transition-colors ${
                      selectedIds?.includes(product?.id) ? "bg-violet-50 hover:bg-violet-100/60" : "hover:bg-slate-50/50"
                    }`}
                  ...
                  >
                    ...
                    {/* Estado — solo visibilidad de catálogo */}
                    <td className="px-4 py-3 text-center">
                      <div className="relative inline-block">
                        <button
                          onClick={() => setOpenStatusMenuId((prev) => prev === product?.id ? null : product?.id)}
                          className="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold transition-colors hover:opacity-80"
                        ...
                        {isStatusMenuOpen && (
                          <div
                            className="absolute left-1/2 -translate-x-1/2 top-full z-20 mt-2 w-40 rounded-xl border bg-white p-1.5 shadow-lg"
                          ...
                          >
                            {STATUS_OPTIONS.map((opt) => (
                              <button
                                key={opt.key}
                                type="button"
                                onClick={() => { setOpenStatusMenuId(null); onChangeStatus(product?.id, opt.key); }}
                                ...
                                style={{
                                  color: commercialState.key === opt.key ? 'var(--color-primary)' : 'var(--color-foreground)',
                                  fontFamily: 'var(--font-caption)',
                                  fontWeight: commercialState.key === opt.key ? 600 : 400,
                                }}
                              >
                    ...
                    <td className="px-4 py-3 text-center">
                      <PosSwitch
                        active={product?.showInPos}
                        onClick={() => onToggleField(product?.id, 'showInPos', !product?.showInPos)}
                      />
                    </td>
                    {/* Acciones */}
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-end gap-2 whitespace-nowrap">
                        <button onClick={() => onEdit(product?.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-800 focus:outline-none" title="Editar" aria-label={`Editar ${product?.name}`}>
                          <Icon name="Pencil" size={14} color="currentColor" />
                        </button>
                        <button onClick={() => onDuplicate(product?.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-800 focus:outline-none" title="Duplicar" aria-label={`Duplicar ${product?.name}`}>
                          <Icon name="Copy" size={14} color="currentColor" />
                        </button>
                        <button onClick={() => onDeleteRequest(product?.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-rose-500 transition-all hover:bg-rose-50 hover:text-rose-600 focus:outline-none" title="Eliminar" aria-label={`Eliminar ${product?.name}`}>
      ...
      {/* ── Card Layout (mobile / tablet / intermediate) ──────────────── */}
              ...
              <div className="flex flex-col gap-y-3 min-w-0">
                ...
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="h-16 w-16 flex-shrink-0 overflow-hidden rounded-xl border border-white bg-slate-100 shadow-sm shadow-slate-200/40">
                    <Image src={product?.image} alt={product?.imageAlt} className="w-full h-full object-cover" width={64} height={64} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col gap-3 min-w-0 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0 flex-1">
                        <p className="text-[15px] font-bold text-slate-950 line-clamp-1" style={{ fontFamily: "var(--font-heading)" }}>{product?.name}</p>
                        {product?.publicCode ? (
                          <p className="text-[11px] font-semibold tracking-wider mt-0.5" style={{ fontFamily: "var(--font-data)", color: "var(--color-muted-foreground)" }}>{product.publicCode}</p>
                        ) : null}
                        <p className="text-xs text-muted-foreground line-clamp-2 mt-1" style={{ fontFamily: "var(--font-caption)" }}>{product?.description}</p>
                        {product?.category ? (
                          <span className="mt-2 inline-flex w-fit items-center px-2.5 py-1 rounded-full text-xs" style={{ backgroundColor: "var(--color-muted)", color: "var(--color-muted-foreground)", fontFamily: "var(--font-caption)" }}>
                            {product?.category}
                          </span>
                        ) : null}
                      </div>
                      <div className="flex shrink-0 flex-row items-center justify-between gap-3 sm:min-w-[150px] sm:flex-col sm:items-end sm:justify-start">
                        <span className="text-xl font-black leading-none whitespace-nowrap text-slate-950" style={{ fontFamily: "var(--font-data)" }}>{formatPrice(product?.price)}</span>
                        <span className="inline-flex flex-shrink-0 items-center whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-semibold" style={{ backgroundColor: commercialState.bg, color: commercialState.color, borderColor: commercialState.border, fontFamily: 'var(--font-caption)' }}>
                          {commercialState.label}
                        </span>
                      </div>
                    </div>
                ...
                <div className="flex flex-col gap-y-2.5 min-w-0">
                  {/* Estado + TPV en la misma línea */}
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1 min-w-0">
                      <button
                        onClick={() => setOpenStatusMenuId((prev) => prev === product?.id ? null : product?.id)}
                        className="flex h-9 w-full min-w-0 items-center justify-between gap-2 rounded-lg border px-3 text-xs font-semibold transition-colors hover:bg-white"
                      ...
                      >
                        <span className="flex items-center gap-1.5 truncate">
                          <span className="inline-block w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: commercialState.color }} />
                          {commercialState.label}
                        </span>
                      ...
                      {isStatusMenuOpen && (
                        <div
                          className="absolute left-0 right-0 top-full z-20 mt-1.5 rounded-xl border bg-white p-1.5 shadow-lg"
                          style={{ borderColor: 'var(--color-border)', boxShadow: '0 18px 40px rgba(15,23,42,0.14)' }}
                        >
                          {STATUS_OPTIONS.map((opt) => (
                            <button
                              ...
                              type="button"
                              onClick={() => { setOpenStatusMenuId(null); onChangeStatus(product?.id, opt.key); }}
                              className="flex min-h-9 w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium transition-colors hover:bg-slate-50"
                              style={{
                                color: commercialState.key === opt.key ? 'var(--color-primary)' : 'var(--color-foreground)',
                                fontFamily: 'var(--font-caption)',
                                fontWeight: commercialState.key === opt.key ? 600 : 400,
                            ...
                            >
                              <Icon name={opt.icon} size={13} color="currentColor" />
                              {opt.label}
                            </button>
                          ))}
                        </div>
                    ...
                    {/* TPV switch en card */}
                    <div
                      className="flex items-center gap-2 h-9 px-3 rounded-lg border shrink-0"
                      style={{ borderColor: 'rgba(203,213,225,0.85)' }}
                    >
                      <span className="text-xs font-medium whitespace-nowrap" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>TPV</span>
                      <PosSwitch
                        active={product?.showInPos}
                        onClick={() => onToggleField(product?.id, 'showInPos', !product?.showInPos)}
                      />
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => onEdit(product?.id)} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-semibold text-slate-800 transition-all hover:bg-white hover:text-slate-950" style={{ borderColor: 'rgba(203,213,225,0.85)', fontFamily: 'var(--font-caption)' }} aria-label={`Editar ${product?.name}`}>
                      <Icon name="Pencil" size={13} color="currentColor" />
                      Editar
                    </button>
                    <button onClick={() => onDuplicate(product?.id)} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg px-3 text-xs font-medium text-slate-600 transition-all hover:bg-white hover:text-slate-800" style={{ fontFamily: 'var(--font-caption)' }} aria-label={`Duplicar ${product?.name}`}>
                      <Icon name="Copy" size={13} color="currentColor" />
                      Duplicar
                    </button>
                    <button onClick={() => onDeleteRequest(product?.id)} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-semibold text-rose-600 transition-all hover:bg-rose-50 hover:text-rose-700" style={{ borderColor: 'rgba(244,63,94,0.24)', fontFamily: 'var(--font-caption)' }} aria-label={`Eliminar ${product?.name}`}>
