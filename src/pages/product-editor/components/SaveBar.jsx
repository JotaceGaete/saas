...
export default function SaveBar({ isEditing, isSaving, saveSuccess, saveDisabled, onSave, onSaveAndNew, onCancel, onContinueAdding, catalogUrl, itemSingular = 'producto' }) {
  const saveButtonDisabled = isSaving || saveDisabled;
  const itemSingularLower = itemSingular.toLowerCase();
  const showPostSaveActions = saveSuccess && Boolean(catalogUrl) && Boolean(onContinueAdding);
  return (
    <div
        ...
        {showPostSaveActions ? (
          <div className="flex items-center gap-2 flex-shrink-0">
            <a
              ...
              className="flex items-center gap-1.5 rounded-lg border px-3.5 py-2 text-sm font-semibold transition-all duration-150 hover:-translate-y-px active:scale-[0.97]"
              style={{
            ...
            >
              <Icon name="Eye" size={14} color="var(--color-foreground)" />
              Ver catálogo
            </a>
            <button
              onClick={onContinueAdding}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white transition-all duration-150 hover:-translate-y-px active:scale-[0.97]"
              style={{
            ...
            >
              ...
              {`Seguir agregando ${itemSingularLower}s`}
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-2 flex-shrink-0">
