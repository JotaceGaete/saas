...
describe('BusinessRegistration — captura y atribución de referral', () => {
  ...
  it('signUp con sesión inmediata: intenta atribución como elegible (usuario nuevo)', async () => {
    ...
    await waitFor(() =>
      expect(attemptPendingAttribution).toHaveBeenCalledWith({ userId: 'user-new', isEligible: true, source: 'signup_immediate_session' })
    );
  });
  ...
  it('signIn (login) exitoso: intenta atribución como NO elegible (usuario existente) — limpia, nunca llama la RPC', async () => {
    ...
    await waitFor(() =>
      expect(attemptPendingAttribution).toHaveBeenCalledWith({ userId: 'user-existing', isEligible: false, source: 'login_existing_user' })
    );
  });
