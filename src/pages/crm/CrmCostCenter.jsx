...
function resolveCostCenterCurrency(currency) {
  return String(currency || DEFAULT_CURRENCY).trim().toUpperCase() || DEFAULT_CURRENCY;
}

function createMoneyFormatter(currency) {
  const currencyCode = resolveCostCenterCurrency(currency);
  return (n) => formatMoney(n, currencyCode);
}
...
function HeroCard({ state, salesToday, dailyCost, currency = DEFAULT_CURRENCY }) {
...
function HoyoCard({ totalCosts, salesMonth, currency = DEFAULT_CURRENCY }) {
  const fmt = createMoneyFormatter(currency);
...
function CalendarDot({ day, month, year, state, sales, varExp, dailyCost, isToday, currency = DEFAULT_CURRENCY }) {
  const fmt = createMoneyFormatter(currency);
...
function HealthCalendar({ month, year, dailySales, dailyCost, dailyVarExpenses, currency = DEFAULT_CURRENCY }) {
  const now = new Date();
...
function ComprasWidget({ purchaseTotals, navigate, currency = DEFAULT_CURRENCY }) {
  const fmt = createMoneyFormatter(currency);
...
function CostosWidget({ totalCostosFijos, navigate, currency = DEFAULT_CURRENCY }) {
  const fmt = createMoneyFormatter(currency);
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center justify-between gap-4">
      <div className="flex items-center gap-3 min-w-0">
...
function IvaSummaryPanel({ salesMonth, purchaseTotals, vatRate, currency = DEFAULT_CURRENCY }) {
  const fmt = createMoneyFormatter(currency);
...
function Dashboard({ center, costItems, sales, dailySales, purchaseTotals, purchases, month, year, onEditSettings, businessId, navigate, currency }) {
  ...
  return (
    <div className="space-y-4 max-w-lg mx-auto">
      ...
      <CostosWidget totalCostosFijos={totalCostosFijos} navigate={navigate} currency={currencyCode} />

      {/* Widget de compras — fuente única de datos */}
      <ComprasWidget purchaseTotals={purchaseTotals} navigate={navigate} currency={currencyCode} />
