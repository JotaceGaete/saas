...
export default function CrmInvoices() {
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter) {
      list = list.filter(inv => isInDateRange(inv.issue_date, dateFilter));
    }

    const q = searchQuery.trim().toLowerCase();
    if (q) {
      list = list.filter(inv => {
        const num     = formatInvoiceNumber(inv.invoice_number)?.toLowerCase() ?? '';
        const name    = inv.wa_customers?.name?.toLowerCase() ?? '';
        const company = inv.wa_customers?.company?.toLowerCase() ?? '';
        const rut     = inv.wa_customers?.rut?.toLowerCase() ?? '';
        const phone   = inv.wa_customers?.phone?.toLowerCase() ?? '';
        const status  = inv.status?.toLowerCase() ?? '';
        const total   = String(inv.total ?? '');
        return num.includes(q) || name.includes(q) || company.includes(q) || rut.includes(q) || phone.includes(q) || status.includes(q) || total.includes(q);
      });
    }
