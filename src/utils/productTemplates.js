/**
 * Templates de productos de ejemplo por rubro.
 *
 * Se usan para sembrar automáticamente el catálogo de un negocio recién
 * configurado (ver src/services/productTemplateService.js). No existe UI de
 * templates: el usuario nunca los "instala", solo los edita después desde
 * /product-editor.
 *
 * Claves de PRODUCT_TEMPLATES: clave interna del template.
 * RUBRO_SLUG_TO_TEMPLATE: mapea wa_rubros.slug → clave de template.
 *
 * Cada producto: name, description, price (entero, moneda local) y las
 * imágenes para wa_products.card_image_url / wa_products.thumbnail_url.
 */
...
export function getTemplateProductsForRubro(rubroSlug) {
  ...
  return templateKey ? (PRODUCT_TEMPLATES[templateKey] ?? []) : [];
}
